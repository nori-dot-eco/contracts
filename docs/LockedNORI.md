...
Based on the mechanics of a wrapped ERC-20 token, this contract layers schedules over the withdrawal  functionality to implement _vesting_ (a revocable grant) and _lockup_ (an irrevocable timelock on utility).  ##### Behaviors and features  ###### Grants  
- _Grants_ define lockup periods and vesting schedules for tokens  
- A single grant per address is supported  ###### Vesting  
...
- Tokens are released linearly from the latest cliff date to the end date of the grant based on the block.timestamp  of each block  ###### Lockup  
...
- Transfer of LockedNORI under lockup is forbidden  ###### Cliffs  
...
- This contract supports a maximum of two distinct cliffs per grant. The effect of fewer cliffs can be achieve by  setting one of both cliff times to the start time or end time, and/or by setting the cliff amount to zero.  ###### Additional behaviors and features  
...
- when a grant is defined, LockedNORI follows the restrictions noted above  ##### Inherits  
...
Claim unlocked tokens and withdraw them to *to* address.
...
<i>All grants must include a lockup schedule and can optionally *also*  include a vesting schedule.  Tokens are withdrawble once they are  vested *and* unlocked.  It is also callable externally (see &#x60;grantTo&#x60;) to handle cases  where tokens are incrementally deposited after the grant is established.</i>
...
Hook that is called before send, transfer, mint, and burn. Used to disable transferring locked nori.
