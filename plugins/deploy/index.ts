...
const getNamedSigners = (hre: HardhatRuntimeEnvironment): NamedSigners => {
...
>({
  ...
  const shouldForceDeployProxy =
    process.env.FORCE_PROXY_DEPLOYMENT || // todo deprecate
    hre.deployments.forcedProxyDeployments?.includes(contractName);
  const shouldDeployProxy =
    contractCode === '0x' ||
    shouldForceDeployProxy ||
  ...
  if (
    ...
    hre.logger.warn(
      `${
        shouldForceDeployProxy === true ? '(FORCED) ' : ''
      }Deploying proxy and instance for ${contractName}`
    );
...
extendEnvironment((hre) => {
  ...
  hre.deployments.deployNonUpgradeable = lazyFunction(
    () => deployNonUpgradeable
  );
  hre.deployments.deployOrUpgradeProxy = lazyFunction(
