...
const MARKET_ABI = [
  ...
  {
    inputs: [],
    name: 'IncorrectSupplyAllocation',
    type: 'error',
  },
  {
    inputs: [],
  ...
  },
  {
    ...
    inputs: [
      ...
      {
        indexed: false,
        internalType: 'contract BridgedPolygonNORI',
    ...
    name: 'ContractAddressesRegistered',
    type: 'event',
  },
  {
    anonymous: false,
  ...
  },
  {
    ...
    name: 'NoriFeeWalletAddressUpdated',
    type: 'event',
  },
  {
    anonymous: false,
  ...
  },
  {
    ...
    name: 'PriorityRestrictedThresholdSet',
    type: 'event',
  },
  {
    anonymous: false,
  ...
  {
    inputs: [],
    name: 'priorityRestrictedThreshold',
    outputs: [
      {
        internalType: 'uint256',
        name: '',
...
exports.handler = async function (event) {
  ...
  const { amount, buyerWalletAddress, graphqlEndpoint, apiSecret } =
    event.request.body;
  if (apiSecret !== event.secrets.apiSecret) return;
  const provider = new DefenderRelayProvider(event);
  const signer = new DefenderRelaySigner(event, provider);
  const store = new KeyValueStoreClient(event);
  const bpNoriContract = new ethers.Contract(
    BPNORI_ADDRESS,
    BPNORI_ABI,
    signer
  );
