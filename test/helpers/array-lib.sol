...
contract UInt256ArrayLibHarness {
  function fillUsingStandardImplementation(
    ...
    for (uint256 i = 0; i < values.length; ++i) values[i] = value;
  }
  ...
  function sliceUsingStandardImplementation(
    ...
    for (uint256 i = 0; i < to; ++i) sliced[i] = values[0 + i];
  }
...
contract AddressArrayLibHarness {
  ...
  function fill(
    ...
    return AddressArrayLib.fill({values: new address[](100), value: value});
  }
}
