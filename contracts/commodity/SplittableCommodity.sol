...
contract SplittableCommodity is MintableCommodity {
  ...
  function split(uint _tokenId, address _to, uint256 _amount) public {
    address supplierProxy = IContractRegistry(erc820Registry).getLatestProxyAddr("Supplier");
    require(
      msg.sender == IContractRegistry(erc820Registry).getLatestProxyAddr("FifoCrcMarket") ||
    ...
    if(msg.sender == IContractRegistry(erc820Registry).getLatestProxyAddr("FifoCrcMarket")) {
