...
interface ICommodity {
  ...
  function isOperatorForOne(address operator, uint256 tokenId) public constant returns (bool);
  function operatorSendOne(address from, address to, uint256 tokenId, bytes userData, bytes operatorData) public;
  // TODO jaycen do we need an ability to transfer all commodities (from operator) to a new owner?
  function operatorSend(address from, address to, uint256 tokenId, bytes userData, bytes operatorData) public;
