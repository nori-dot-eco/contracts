 ...
 * - Transfer of VestingNORIPreset under lockup is forbidden
 *
 * ###### Cliffs
 *
 ...
 *   - In absence of a grant VestingNORIPreset functions identically to a standard wrapped token
 *   - when a grant is defined, VestingNORIPreset follows the restrictions noted above
 *
 * ##### Inherits
 *
  ...
  ERC777PresetPausablePermissioned
{
   ...
   * ERC-1820 registry
   *
   * @dev Registering that VestingNORIPreset implements the ERC777TokensRecipient interface with the registry is a
   ...
   *
   * @dev This function burns `amount` of `VestingNORIPreset` and transfers `amount`
   * of `BridgedPolygonNORI` from the `VestingNORIPreset` contract's balance to
   ...
   *
   * Unlike in the `claim` function, here we burn `VestingNORIPreset` from the grant holder but
